...
func (f *FIleLogger) log(level Level, format string, args ...interface{}) {
	...
	// 将日志信息发送通道中， 以备写日志的goroutine去接收值
	//  1 先构造日志数据结构体
	logdata := &logData{
		Massage:  msg,
		LogLevel: logLevelSt,
...
func (f *FIleLogger) writeLogBackgroud() {
	for Data := range f.logDataChan {
		...
		logMsg := fmt.Sprintf("[%s] [%s:%d] [%s] [%s] %s", Data.TimeStr, Data.FileName, Data.LineNo, Data.FuncName, Data.LogLevel, Data.Massage)
		// 往文件里面写之前检查
